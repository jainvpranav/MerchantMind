import pandas as pd, psycopg2, uuid
...
df = pd.DataFrame(cur.fetchall(), columns=['customer','last_seen','frequency','monetary','avg_basket'])
now = datetime.now()

df['recency_days'] = (now - pd.to_datetime(df['last_seen'])).dt.days
...
for _, row in df.iterrows():
    cur.execute(
        ...
        (str(uuid.uuid4()), MERCHANT_ID, row['customer'], row['segment'],
         row['last_seen'], float(row['avg_basket']), int(row['frequency']), datetime.now())
    )
conn.commit()
